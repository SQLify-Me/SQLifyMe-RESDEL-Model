...
def _main():
    input_dir = "./demo/new_DB"
    output_file = "./demo/tables.json"
    output_file_new = "./demo/tables_new.json"

    #ex_tab_file = "./demo/tables.json"
    ...
    with open(output_file_new, 'wt') as out:
    ...
    existing_data.extend(tables)  # Assuming new_data is a list of dictionaries

    with open(output_file, 'w') as f:
        json.dump(existing_data, f, sort_keys=True, indent=2, separators=(',', ': '))

    end_time = time.perf_counter()  # Get end time

    elapsed_time = end_time - start_time
    print(f"Execution time readinggggggggggggggggg: {elapsed_time:.6f} seconds")