...
if __name__ == '__main__':
    ...
    all_words = pickle.load(open(os.path.join("D:/mohamed/REDSQL/RESDSQL-main/RESDSQL-main/NatSQL/data/20k.pkl"), 'rb'))
    new_tables = []
    ...
    if args.table_transform:
       ...
       # json.dump(new_tables,open(args.out_file,'w'), indent=2)
        ...
        # Ensure existing_data is a list (handle potential errors)
        if not isinstance(existing_data, list):
            raise TypeError("Existing data in the file must be a list")

        # Append new data to the existing list
        existing_data.extend(new_tables)
